...
function onclick( document, elm, event )
{
 ...
 if( NEXT_CLICK_MEMORY == 'y' )
 {
  navigator.clipboard.writeText( id ).then( function(document) { }, function(document) { } );
  NEXT_CLICK_MEMORY = false; return
 }
...
,
y : { text : "[y]ank (copy) node ID of next click to clipboard (switch to visual mode)",	f : function(document)
    {
      execute_keyboard_function( document, 'v' )
      // yanking itself will be performed by onclick handler; the y key has been saved automatically
    }}
,
',':{ text : "copy selection to clipboard, separated by [,] (comma)", f : function(document) { copy_selection_to_clipboard( document, ',' ) } }
,
' ':{ text : "copy selection to clipboard, separated by [ ] (space)", f : function(document) { copy_selection_to_clipboard( document, ' ' ) } }


,
'?':{ text : "show this help window",						f : function(document)
      {
...
,
'Escape':{ text : "close this help window",						f : function(document)
      {
...
function generateKeyboardShortcutButtons()
{
 ...
 buttonContainer.id = "keyboardshortcuts"
 buttonContainer.classList.add( "modal-content" )
 buttonContainer.innerHTML = 'KTS Commands'
 //buttonContainer.appendChild( document.createElement('<div class="close" onclick="modal.style.display=\'none\'" >&times;</div>') )
 ...
 {
   node = document.getRootNode().children[0].appendChild(  document.createElement( 'foreignObject' )  )
 }
